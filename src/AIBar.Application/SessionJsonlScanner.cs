...
public sealed record SessionCheckpoint(long Identity, long Length, long LastWriteUtcTicks, long Offset, string ParserVersion);
...
public sealed class SessionJsonlScanner(SessionCheckpointStore checkpoints, string parserVersion, Action? beforeCommit = null)
{
    ...
    {
        cancellationToken.ThrowIfCancellationRequested();
        if (!TrySnapshot(path, out var initial)) return Result([], ["session_file_changed"]);
        var checkpoint = await checkpoints.LoadAsync(path, cancellationToken);
        var rebuild = checkpoint is not null && (!ValidOffset(path, checkpoint.Offset, checkpoint.Length, initial.Length) ||
            checkpoint.Identity != initial.Identity || checkpoint.Length > initial.Length || checkpoint.ParserVersion != parserVersion ||
            (checkpoint.Length == initial.Length && checkpoint.LastWriteUtcTicks != initial.LastWriteUtcTicks));
        if (checkpoint is not null && !rebuild && checkpoint.Length == initial.Length && checkpoint.LastWriteUtcTicks == initial.LastWriteUtcTicks)
            return Result([], checkpoint.Offset < checkpoint.Length ? ["session_incomplete_tail"] : []);
        ...
        try
        {
            ...
            while (stream.ReadByte() is var next && next >= 0)
            {
                ...
                Parse(line, records, warnings);
                line.Clear();
            }
            if (line.Count > 0) warnings.Add("session_incomplete_tail");
            beforeCommit?.Invoke();
            cancellationToken.ThrowIfCancellationRequested();
            if (!Stable(path, initial)) return Result([], warnings.Append("session_file_changed"), rebuild);
            await checkpoints.CommitAsync(path, new(initial.Identity, initial.Length, initial.LastWriteUtcTicks, committedOffset, parserVersion), cancellationToken);
            return Result(records, warnings, rebuild);
        }
        catch (IOException) { return Result([], warnings.Append("session_file_changed"), rebuild); }
    }
