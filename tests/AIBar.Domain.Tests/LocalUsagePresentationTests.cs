...
public sealed class LocalUsagePresentationTests
{
    ...
    public void Maps_loaded_disabled_empty_partial_problem_and_unavailable_states_truthfully()
    {
        ...
        Assert.All(skipped.Rows, row => { Assert.Null(row.Total); Assert.Null(row.TokenBreakdownLabel); Assert.DoesNotContain("Input ", row.AutomationLabel, StringComparison.Ordinal); });

        var empty = mapper.Map(Result(LocalUsageProjectionStatus.Completed, []));
        Assert.Equal("No retained usage facts", empty.StatusLabel);
        Assert.All(empty.Rows, row => Assert.Null(row.TokenBreakdownLabel));
        ...
        Assert.All(unavailable.Rows, row => { Assert.Null(row.Total); Assert.Null(row.TokenBreakdownLabel); });

        var notLoaded = mapper.NotLoaded();
        Assert.All(notLoaded.Rows, row => Assert.Null(row.TokenBreakdownLabel));
    }
