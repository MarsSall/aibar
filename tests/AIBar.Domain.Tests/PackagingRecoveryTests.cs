...
public sealed class PackagingRecoveryTests
{
    ...
    private sealed class DescendantHarness : IDisposable
    {
        ...
        private static readonly string[] RequiredHarnessOutputFiles =
        [
            "Harness.exe",
            "Harness.dll",
            "Harness.deps.json",
            "Harness.runtimeconfig.json"
        ];
        ...
        {
            var harnessOutput = PrebuiltHarnessOutput();
            if (!Directory.Exists(harnessOutput))
                throw new DirectoryNotFoundException($"Prebuilt PackagingRecovery process harness output was not found at '{harnessOutput}'. Build AIBar.Packaging.ProcessHarness before running these tests.");

            foreach (var fileName in RequiredHarnessOutputFiles)
            {
                var source = Path.Combine(harnessOutput, fileName);
                if (!File.Exists(source))
                    throw new FileNotFoundException($"Required prebuilt PackagingRecovery process harness file was not found: '{source}'. Build AIBar.Packaging.ProcessHarness before running these tests.", source);
            }

            var root = Directory.CreateDirectory(Path.Combine(Path.GetTempPath(), $"aibar descendant {Guid.NewGuid():N} café")).FullName;
            foreach (var fileName in RequiredHarnessOutputFiles)
                File.Copy(Path.Combine(harnessOutput, fileName), Path.Combine(root, fileName));
            ...
            return new(root, Path.Combine(root, "Harness.exe"), Path.Combine(root, "identity.json"), Path.Combine(root, "arguments.txt"), Path.Combine(root, ".owned-marker"), nonce, readyName, releaseName, ready, release);
        }

        private static string PrebuiltHarnessOutput()
        {
            var testAssemblyPath = typeof(PackagingRecoveryTests).Assembly.Location;
            if (string.IsNullOrWhiteSpace(testAssemblyPath))
                throw new InvalidOperationException("Cannot locate the prebuilt PackagingRecovery process harness because the test assembly output path is unavailable.");

            var testAssemblyDirectory = new FileInfo(testAssemblyPath).Directory!;
            var targetFrameworkDirectory = testAssemblyDirectory;
            while (targetFrameworkDirectory is not null && !string.Equals(targetFrameworkDirectory.Name, "net8.0-windows", StringComparison.OrdinalIgnoreCase))
                targetFrameworkDirectory = targetFrameworkDirectory.Parent;

            if (targetFrameworkDirectory is null || targetFrameworkDirectory.Parent is null || string.IsNullOrWhiteSpace(targetFrameworkDirectory.Parent.Name))
                throw new InvalidOperationException($"Cannot determine the test build configuration from assembly output path '{testAssemblyPath}'. Expected a net8.0-windows output directory.");

            var configuration = targetFrameworkDirectory.Parent.Name;
            var harnessOutput = Path.Combine(RepositoryRoot(), "tests", "AIBar.Packaging.ProcessHarness", "bin", configuration, "net8.0-windows");
            var runtimeSpecificOutput = Path.GetRelativePath(targetFrameworkDirectory.FullName, testAssemblyDirectory.FullName);
            return runtimeSpecificOutput == "." ? harnessOutput : Path.Combine(harnessOutput, runtimeSpecificOutput);
        }
