...
| Field | Value |
|-------|-------|
| Estimated changed lines | C1c: 220–320; C2a: 300–390; C2b: 300–390; separate units |
| 400-line budget risk | High overall; C1c implementation completed under 400, while independent C1c verification and future C2a/C2b retain mandatory >400 workload visibility/review-splitting pressure |
| Chained PRs recommended | Yes |
| Suggested split | `03ee654` → `b2c-C1c` → `b2c-C2a` → `b2c-C2b`; feature-branch-chain; no combined C2 PR; C3 remains excluded |
| Native changed-line ceilings | C1c implementation history: 400, no exception; independent C1c verification: 1000; future C2a: 1000; future C2b: 1000 |
...
C1c implementation retained its 220–320 authored additions/deletions forecast, High implementation risk, Medium 400-line risk, hard cap 400 with no exception, and separate prerequisite/review boundary. Completed apply Attempt 62 is historical at 347/400 with evidence `sha256:d7d8bd1cae69d55c820c2c06dd1c2e137fee47e8fd9a3a7930d806b778dddec8`; it must not be reset, reused, or reclassified as verification. The next parent action is a distinct independent C1c verification objective with max attempts 1 and native changed-line ceiling 1000, requiring fresh native reset/begin authorization. It verifies the exact Attempt 62 candidate/evidence, makes no functional code edits by default, and writes only verification evidence/report unless a separate correction decision occurs.

Producer is complete and committed at `03ee654`. C2a and C2b each remain 300–390 authored additions/deletions, High risk, with a native changed-line ceiling of 1000. C2a remains blocked on independently verified and committed C1c and may consume only the retained-tree facet. C2b remains blocked on committed C1c plus independently verified and committed C2a. Crossing 400 remains mandatory workload visibility and review-splitting pressure for both future units. The 1000 ceilings provide evidence/correction headroom only and do not expand behavior, allowed paths, dependencies, absorb another unit, merge C1c/C2a/C2b, authorize C3, or authorize stage/commit/push/PR/review. Generated fixtures remain bound to the behavior they verify and do not create scope authority. Attempts 60 and 61 are failed historical evidence only; they must not be reset, retried, resumed, copied, or reused.
...
- [x] Plan one fresh native implementation objective for C1c only with proposed max attempts 1 and max changed lines 400; begin only after explicit maintainer apply approval and fresh `gentle-ai sdd-attempt status --cwd <repo> --change aibar-foundation` confirms `next_action=begin`, without reusing Attempts 60/61. <!-- sdd-owner: parent -->
- [x] Create or change `feature/aibar-foundation-slice-8c1-1b2c-c1c-facets` only after maintainer apply approval; perform no stage/commit/push/PR/review without separate maintainer authorization, and leave native-ledger operations and any C2a/C2b advancement to separately authorized parent phases. <!-- sdd-owner: parent -->
...
##### b2c-C2a — Trusted retained-handle operations (300–390 authored lines)

**Dependency/base:** C2a is visibly blocked until `b2c-C1c` is independently verified and committed on top of producer `03ee654` (tree `e64bcd8f4b46948ead3feaf8eca6a1dc5e1fef63`). It may accept only one still-live `RetainedTreeCapabilityFacet` from that committed C1c handoff and MUST reject the direct/legacy combined producer capability. C2b remains blocked on committed C1c plus committed C2a. **Allowed paths:** `tools/AIBar.Packaging.Supervisor/DirectoryCapability.cs`, `tests/AIBar.Domain.Tests/PackagingSupervisorTests.cs`, `openspec/changes/aibar-foundation/tasks.md`, and `openspec/changes/aibar-foundation/apply-progress.md`. This is the narrowest realistic path set because the current native capability, Windows P/Invoke surface, and test fakes are centralized in `DirectoryCapability.cs` and `PackagingSupervisorTests.cs`. **Start:** independently verified C1c transfers one still-live retained-tree facet while the evidence facet remains separately owned. **End:** a session-bound mechanism layer exposes only bounded native retained-handle operations; no evidence policy, DPAPI metadata, authorization minting, production cleanup, C2b behavior, or C3 behavior is present. **Forbidden:** direct/legacy combined producer input, path-selected or reconstructed roots, arbitrary handles, final-path authority, `CommittedChildEvidence` inspection, HMAC/digest policy, DPAPI, unguarded deletion, Win32/managed/path fallback, shell, PowerShell, subprocess, copy-delete, rename-back, scavenging, `.gitignore`, or `.git/gentle-ai` changes.
...
**Acceptance evidence:** native layout/parser and status matrix; committed-C1c handoff/session/lease ownership tests; focused/full/build/diff results; Windows 10/11 x64 runtime proof; process/root/handle cleanup evidence; and source inspection proving no direct/legacy combined input, path authority, evidence-policy input, DPAPI, unguarded delete, or fallback. **Review boundary:** one isolated feature-branch-chain unit targeting independently verified and committed C1c; forecast 300–390 authored additions/deletions; risk High; maintainer-approved native changed-line ceiling 1000. The additional ceiling is evidence/correction headroom only; it cannot expand behavior, allowed paths, dependency scope, absorb C1c/C2b/C3, merge units, or authorize stage/commit/push/PR/review. **Decision before apply:** Yes — only after C1c is independently verified and committed, with separate maintainer approval for the immediate C2a apply phase and fresh native status confirming `next_action=begin`; existing C2a scope/ceiling approval does not authorize skipping that phase gate. Native-ledger operations remain parent-owned and are not performed by this planning amendment. **Rollback:** only if C2b is absent, disable/remove C2a session/interop/tests and its task/progress evidence, preserve committed C1c and producer `03ee654`, and preserve all historical attempt evidence.

##### b2c-C2b — Exact-bijection guarded cleanup (300–390 authored lines)

**Dependency/base:** independently verified, reviewed, and committed C1c plus independently verified, reviewed, and committed C2a; consumes exactly one C2a session and exactly one matching `CommittedEvidenceCapabilityFacet` from the same committed C1c handoff. It MUST reject the direct/legacy combined producer capability, evidence-only reconstruction, and cross-handoff pairing. **Allowed paths:** `tools/AIBar.Packaging.Supervisor/Cleanup.cs`, `tools/AIBar.Packaging.Supervisor/ScavengerMetadata.cs`, `tests/AIBar.Domain.Tests/PackagingSupervisorTests.cs`, `openspec/changes/aibar-foundation/tasks.md`, and `openspec/changes/aibar-foundation/apply-progress.md`. **Start:** committed C1c preserves the immutable evidence facet and committed C2a provides trusted retained-handle mechanics. **End:** exact-bijection guarded cleanup either completes with verified disposal or truthfully retains the quarantine as `CLEANUP_PARTIAL`; no combined C2 unit, path discovery, arbitrary-root selection, C3 scavenging, or PowerShell integration is included. **Forbidden:** producer weakening/removal, direct/legacy combined input, evidence synthesis, cross-handoff pairing, legacy path-only cleanup, caller-selected roots, path-based enumeration/reopen/delete, shell/helper fallback, copy-delete, rename-back, target-changing retry, original-path preservation after commit, `.gitignore`, or `.git/gentle-ai` authority changes.

- [ ] **RED:** Add failing tests for exact count/tag/identity/kind/volume/non-reparse/digest bijection while leases remain live, with missing/extra/renamed/substituted/duplicate/reparse/cross-volume/inaccessible/unknown-kind/tag/identity/digest/cancellation/timeout/enumeration/reopen faults and a zero-deletion counter. Add DPAPI `CurrentUser` tests for missing, legacy, corrupt, wrong-version, wrong-digest, wrong-correlation, wrong-root, unprotectable, non-durable, unbound, and retry-exhausted metadata; cover faults before the first deletion and after deletion starts. <!-- sdd-owner: implementation -->
- [ ] **GREEN:** Implement the C2b consumer across `Cleanup.cs` and `ScavengerMetadata.cs`: consume only the independently committed C2a session and matching producer evidence; retain complete direct-child leases; derive HMAC tags and handle facts in bounded memory; require exact bijection before deletion; serialize only bounded retry metadata, protect/unprotect/constant-time verify it with DPAPI `CurrentUser`, durably retain only protected bytes, zero plaintext/keys/tags/digests/temporary records, then mint C2a's one-use session-bound authorization and perform bounded handle-anchored recursive post-order deletion with immediate re-observation. Map every post-commit uncertainty to truthful `CLEANUP_PARTIAL`; never discover or select a root from metadata. <!-- sdd-owner: implementation -->
- [ ] **TRIANGULATE/REFACTOR:** Prove no tag-only, leaf-spelling, enumeration-order, fresh-enumeration, or path authority; no deletion occurs for any pre-first-deletion uncertainty; every pre/post-start fault retains the quarantine with truthful `CLEANUP_PARTIAL`; leases, handles, DPAPI buffers, evidence keys, and plaintext are disposed/zeroed exactly once; retries never rename back or choose an arbitrary root; recursive descendants remain governed by handle-relative identity/reparse checks. Keep C3 discovery, scheduling, owner/ACL/age admission, and PowerShell out of scope. <!-- sdd-owner: implementation -->
- [ ] **GATE:** Run focused `dotnet test tests/AIBar.Domain.Tests/AIBar.Domain.Tests.csproj --filter "FullyQualifiedName~PackagingSupervisor|FullyQualifiedName~CommittedChildEvidence|FullyQualifiedName~Cleanup" --no-restore -m:1 --nologo`, full `dotnet test AIBar.sln --no-restore -m:1 --nologo`, `dotnet build AIBar.sln --no-restore --nologo`, and `git diff --check`. Execute the Windows 10/11 x64 runtime matrix for successful exact-bijection guarded recursive deletion, injected faults before the first deletion and after deletion starts, truthful retained partials, crash-window metadata handling, and cleanup after handle disposal. Record zero helper/process residue, zero leaked handles/buffers, zero unintended root residue, and no C3/PowerShell execution. Crossing 400 requires mandatory workload visibility and review-splitting pressure; stop and replan before unsafe behavior or above the native changed-line ceiling of 1000. <!-- sdd-owner: implementation -->

**Acceptance evidence:** direct-child bijection and deletion-counter matrix; DPAPI CurrentUser protected-byte/status evidence; pre-first-deletion zero-mutation proof; pre/post-start retained-partial evidence; zeroization/disposal inspection; focused/full/build/diff/runtime results; and path/secret-free diagnostics. **Review boundary:** one chained feature-branch-chain unit targeting the independently committed C2a branch, with committed C1c as a required predecessor; forecast 300–390 authored additions/deletions; risk High; maintainer-approved native changed-line ceiling 1000. The additional ceiling is evidence/correction headroom only; it cannot expand behavior, allowed paths, dependency scope, absorb C1c/C2a/C3, merge units, or authorize stage/commit/push/PR/review. **Decision before apply:** Yes — only after C1c and C2a are independently verified and committed; the parent must confirm the exact C2b scope. **Rollback:** disable/remove C2b policy, DPAPI metadata, authorization consumption, guarded cleanup, tests, and task/progress evidence first; then remove C2a; remove C1c; remove producer `03ee654` last. Never leave a consumer active without its predecessor.
...
| --- | ---: | --- | ---: | --- |
| Producer commit `03ee654` | Complete, unchanged | Complete | N/A | Base |
| `b2c-C1c` atomic capability-facet transfer | 220–320 implementation; completed Attempt 62 at 347/400 | High; implementation 400-line risk was monitored | 400 historical implementation; 1000 independent verification | PR 1: next objective is independent verification of exact Attempt 62 candidate/evidence; no functional edits by default |
| `b2c-C2a` trusted retained-handle operations | 300–390 | High | 1000 approved | PR 2: blocked on independently verified/committed C1c; >400 is mandatory workload visibility/review-splitting pressure, not scope authority |
| `b2c-C2b` exact-bijection guarded cleanup | 300–390 | High | 1000 approved | PR 3: blocked on committed C1c plus committed C2a; >400 is mandatory workload visibility/review-splitting pressure, not scope authority |
| Remaining C1c→C2 chain | 820–1,100 | High | Separate per-unit caps; no combined scope | No single combined PR |

Decision needed before producer apply: No — producer is complete at `03ee654`
Decision needed before C1c independent verification: Yes — obtain fresh native reset/begin authorization for a distinct max-attempts-1 objective at ceiling 1000; do not reset/reuse Attempt 62
Decision needed before C2a apply: Yes — only after C1c is independently verified and committed; native-ledger operations remain parent-owned
Decision needed before C2b apply: Yes — only after C1c and C2a are independently verified and committed
Chained PRs recommended: Yes
Chain strategy: feature-branch-chain
400-line budget risk: historical C1c implementation completed at 347/400; crossing 400 remains mandatory workload visibility/review-splitting pressure for future C2a/C2b, while their 1000 ceilings grant evidence/correction headroom only
Delivery strategy: ask-on-risk
No combined C2 PR; C3 remains excluded

#### Remaining Work Forecast for the Parent

1. **Immediate b2c cleanup chain after the producer:** producer commit `03ee654` is complete. The next parent action is a distinct independent C1c verification objective with max attempts 1 and native changed-line ceiling 1000. It must freshly reset/begin native authority, verify the exact completed Attempt 62 candidate/evidence `sha256:d7d8bd1cae69d55c820c2c06dd1c2e137fee47e8fd9a3a7930d806b778dddec8`, make no functional code edits by default, and write only verification evidence/report unless a separate correction decision occurs. Attempt 62 remains historical at 347/400 and cannot be reset/reused as verification. After C1c is independently verified and committed, `b2c-C2a` remains 300–390 authored lines and High risk with a 1000 ceiling; after committed C1c and C2a, `b2c-C2b` remains 300–390 and High risk with a 1000 ceiling. Each is a separate feature-branch-chain unit; there is no combined C2 PR. Crossing 400 remains mandatory workload visibility/review-splitting pressure for C2a/C2b. Every 1000 ceiling provides evidence/correction headroom only and cannot expand behavior, allowed paths, dependencies, absorb another unit, merge C1c/C2a/C2b, authorize C3, or authorize stage/commit/push/PR/review. Attempts 60 and 61 are terminal historical failures only: Attempt 60 evidence `sha256:b75015c735c059de4720f6eb5705d3e21e0f98811e1acd138842d1f3602e011f`, tree `a8fcdab1d34fcfbdd171a81643f58b7cb34b0533`, 301/400 lines; Attempt 61 evidence `sha256:b9c694750e1d4f58a76c096e3875922a66a38a72963ca37195ac68cdb46af974`, tree `68c48e4aab5b21d0bfdab860169468cf18062247`, 341/800 lines. Never reset, retry, resume, copy, or reuse either failed attempt. C3 is excluded, not a remaining unit in this chain.
...
- [ ] Preserve the completed producer commit `03ee654` and its exact candidate/receipt boundary; do not reapply or reopen the producer while advancing C2a/C2b. <!-- sdd-owner: parent -->
- [ ] Start or reuse the bounded review for C2a only after C1c is independently verified and committed; keep >400 as mandatory workload visibility/review-splitting pressure only, accept only the retained-tree facet, preserve producer commit `03ee654`, freeze the C2a candidate, and leave native-ledger operations to the parent. <!-- sdd-owner: parent -->
- [ ] Start or reuse the bounded review for C2b only after C1c and C2a are independently verified, reviewed, and committed, and the C2b candidate is frozen. <!-- sdd-owner: parent -->
- [ ] Enforce producer → C1c → C2a → C2b feature-branch-chain ordering and reverse rollback ordering at each apply/verification/review/commit gate; preserve C1c implementation Attempt 62 at 347/400, use 1000 only for the distinct independent C1c verification objective and future C2a/C2b native objectives, retain mandatory >400 workload visibility/review-splitting pressure, and do not reset, retry, resume, copy, or reuse failed Attempts 60/61 or reset/reuse Attempt 62 as verification. <!-- sdd-owner: parent -->
